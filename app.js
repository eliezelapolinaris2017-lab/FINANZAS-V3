...
/* ====== ⬇️ ACTUALIZADO: HOME con gráfica 12M ACUMULADA ⬇️ ====== */
function renderHome(){
  const now=new Date(); 
  const monthStart=new Date(now.getFullYear(), now.getMonth(), 1).toISOString().slice(0,10); 
  const today=now.toISOString().slice(0,10);

  // KPIs del mes (se mantienen)
  const incMonth=sumRange(state.incomesDaily, monthStart, today);
  ...
  // 1) Cálculo mensual (no acumulado todavía)
  const months=[], incMonthly=[], expMonthly=[];
  for(let i=11;i>=0;i--){
    ...
    incMonthly.push(incM);
    expMonthly.push(expSplitM.total+perM+payM);
  }

  // 2) Convertir a ACUMULADO (running sum) para evitar “arranque en 0” del mes
  const incAcc=[], expAcc=[];
  let ai=0, ae=0;
  for(let k=0;k<months.length;k++){
    ai+=incMonthly[k]; incAcc.push(ai);
    ae+=expMonthly[k]; expAcc.push(ae);
  }
  ...
  months.forEach((m,idx)=>{
    ...
    ctx.fillStyle='#C7A24B'; ctx.fillRect(x,c.height-10-hI,barW,hI);     // Ingresos acumulados
    ctx.fillStyle='#555';    ctx.fillRect(x+barW+4,c.height-10-hE,barW,hE); // Egresos acumulados
    ctx.fillStyle='#aaa'; ctx.font='12px system-ui'; ctx.fillText(m,x,c.height-2);
  });
}
/* ====== ⬆️ FIN CAMBIO renderHome acumulado ⬆️ ====== */
...
/* =========================================================
   Conciliación: contar Coincidencias / No encontradas
   ========================================================= */
(function () {
  const $ = (id) => document.getElementById(id);

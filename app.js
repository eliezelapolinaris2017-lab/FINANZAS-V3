/* =========================================================
   Nexus Finance — app.js
   Core, Estado, Tema, Login estable, SPA, Firebase, PDF
   ========================================================= */
...
function ask(curr,label){
  const v = prompt(label, curr??'');
  if(v===null) return {cancelled:true,value:curr};
  return {cancelled:false,value:v};
}
function askNumber(curr,label){
  const a = ask(String(curr??''),label);
  if(a.cancelled) return a;
  const n = parseFloat(String(a.value).replace(',','.'));
  if(Number.isNaN(n)) return {cancelled:true,value:curr};
...
function applyTheme(){
  ...
  ['brandLogo','logoPreview'].forEach(id=>{
    const el = $('#'+id);
    if(el) el.src = state.settings.logoBase64 || FALLBACK_LOGO;
  });
}

/**
 * SPA: solo una .view.visible (excepto login que se maneja aparte)
 */
...
// Hacer accesible desde HTML (por si quieres llamarlo)
...
function forceShowLogin(){
  ...
  $$('.view.visible').forEach(v=>{
    if(v.id!=='login') v.classList.remove('visible');
  });
...
async function handleLogin(ev){
  ...
  if(createMode){
    if(pin.length<4 || pin.length>8){
      toast('El PIN debe tener entre 4 y 8 dígitos');
      return;
    }
    if(pin!==pin2){
  ...
  if(attempts()>=MAX_ATTEMPTS){
    toast('Has alcanzado el máximo de intentos');
    return;
  }
...
function updateLoginUI(){
  const createMode = !state.settings.pinHash;
  if($('#loginTitle')) $('#loginTitle').textContent = createMode ? 'Crear PIN' : 'Ingresar PIN';
  if($('#loginHint'))  $('#loginHint').textContent  = createMode ? 'Crea un PIN de 4–8 dígitos.' : 'Introduce tu PIN.';
  ...
  if($('#loginAttempts')){
    $('#loginAttempts').textContent = createMode ? '' : `Intentos restantes: ${left}`;
  }
...
function renderExpenses(){
  ...
  if($('#expSumTotal')) $('#expSumTotal').textContent = fmt(total);
  ...
  if(pills){
    ...
    Object.entries(cats).forEach(([k,v])=>{
      ...
      span.innerHTML = `<span>${k}</span><span class="value">${fmt(v)}</span>`;
      pills.appendChild(span);
    });
  }

  $$('#expensesTable [data-del]').forEach(b=>{
    b.onclick = () =>{
      state.expensesDaily = state.expensesDaily.filter(x=>x.id!==b.dataset.del);
      save();
      toast('Gasto eliminado');
    };
  });
  $$('#expensesTable [data-edit]').forEach(b=>{
    b.onclick = ()=> editExpense(b.dataset.edit);
  });
}
function editExpense(id){
  const i = state.expensesDaily.findIndex(x=>x.id===id);
  if(i<0) return;
...
function renderIncomes(){
  ...
  const totalsByMethod = {
    'Efectivo':0,'Tarjeta':0,'Cheque':0,'ATH Móvil':0,'Transferencia':0
  };
  ...
  if($('#incSumTotal')) $('#incSumTotal').textContent = fmt(total);
  ...
  if(methodWrap){
    ...
    Object.entries(totalsByMethod).forEach(([method, value])=>{
      const span=document.createElement('span');
      span.innerHTML = `<span>${method}</span><span class="value">${fmt(value)}</span>`;
      methodWrap.appendChild(span);
    });
  }
...
function renderPayments(){
  ...
  // Resumen de nómina (tarjetas arriba)
  const total   = (totals['Pendiente']||0) + (totals['Pagado']||0);
  const paid    = (totals['Pagado']||0);
  const pending = (totals['Pendiente']||0);

  if($('#payrollTotal'))   $('#payrollTotal').textContent   = fmt(total);
  if($('#payrollPaid'))    $('#payrollPaid').textContent    = fmt(paid);
  if($('#payrollPending')) $('#payrollPending').textContent = fmt(pending);

  $$('#paymentsTable [data-del]').forEach(b=>{
    b.onclick=()=>{
      state.payments = state.payments.filter(x=>x.id!==b.dataset.del);
      save();
      toast('Pago eliminado');
    };
  });
  $$('#paymentsTable [data-edit]').forEach(b=>{
    b.onclick=()=> editPayment(b.dataset.edit);
  });
}
...
function renderOrdinary(){
  ...
  if($('#ordSumCount')) $('#ordSumCount').textContent = state.ordinary.length.toString();
  const next = state.ordinary.map(o=>o.next).filter(Boolean).sort()[0] || '—';
  if($('#ordSumNext')) $('#ordSumNext').textContent = next;

  $$('#ordinaryTable [data-del]').forEach(b=>{
    b.onclick=()=>{
      state.ordinary = state.ordinary.filter(x=>x.id!==b.dataset.del);
      save();
      toast('Recurrente eliminado');
    };
  });
  $$('#ordinaryTable [data-edit]').forEach(b=>{
    b.onclick=()=> editOrdinary(b.dataset.edit);
  });
}
function editOrdinary(id){
  const i=state.ordinary.findIndex(x=>x.id===id);
  if(i<0) return;
...
function wireOrdinary(){
  $('#ordinaryForm')?.addEventListener('submit',(ev)=>{
    ...
    const rec={
      id:uid(),
      name:$('#ordName')?.value,
      amount:Number($('#ordAmount')?.value||0),
      freq:$('#ordFreq')?.value,
      next:$('#ordNext')?.value
    };
    if(!rec.next) return toast('Próxima fecha requerida');
    state.ordinary.push(rec);
    save();
...
function renderBudgets(){
  ...
  state.budgets.forEach(b=>{
    ...
    div.innerHTML=`
      <div class="label">
        ...
        <span>${fmt(used)} / ${fmt(b.limit)} (${pct}%)</span>
      </div>
  ...
  $$('#budgetBars [data-del]').forEach(b=>{
    b.onclick=()=>{
      state.budgets = state.budgets.filter(x=>x.id!==b.dataset.del);
      save();
      toast('Presupuesto eliminado');
    };
  });
  $$('#budgetBars [data-edit]').forEach(b=>{
    b.onclick=()=> editBudget(b.dataset.edit);
  });
}
function editBudget(id){
  const i=state.budgets.findIndex(x=>x.id===id);
  if(i<0) return;
...
function wireBudgets(){
  $('#budgetForm')?.addEventListener('submit',(ev)=>{
    ...
    const rec={
      id:uid(),
      category:$('#budCategory')?.value,
      limit:Number($('#budLimit')?.value||0)
    };
...
function renderPersonal(){
  ...
  if($('#perSumTotal')) $('#perSumTotal').textContent = fmt(total);

  $$('#personalTable [data-del]').forEach(b=>{
    b.onclick=()=>{
      state.personal = state.personal.filter(x=>x.id!==b.dataset.del);
      save();
      toast('Gasto personal eliminado');
    };
  });
  $$('#personalTable [data-edit]').forEach(b=>{
    b.onclick=()=> editPersonal(b.dataset.edit);
  });
}
function editPersonal(id){
  const i=state.personal.findIndex(x=>x.id===id);
  if(i<0) return;
  ...
  save();
  toast('Gasto personal actualizado');
}
...
function wireInvoicesCreate(){
  ...
  $('#addInvoiceToday')?.addEventListener('click', ()=>{
  ...
  $('#invoiceForm')?.addEventListener('submit',(ev)=>{
    ...
    save();
    toast('Factura creada y registrada en Ingresos');

    ev.target.reset();
    $('#invItemsTable tbody').innerHTML='';
    $('#invSubtotal').textContent='—';
    $('#invTaxTotal').textContent='—';
    $('#invGrandTotal').textContent='—';
  });
}
function renderInvoicesHistory(q=''){
  const tb=$('#invoicesTable tbody');
  if(!tb) return;
  tb.innerHTML='';
  ...
  $$('#invoicesTable [data-del]').forEach(b=>{
    b.onclick=()=> deleteInvoice(b.dataset.del);
  });
  $$('#invoicesTable [data-edit]').forEach(b=>{
    b.onclick=()=> editInvoiceBasic(b.dataset.edit);
  });
  $$('#invoicesTable [data-pdf]').forEach(b=>{
    b.onclick=()=> generatePDF('invoices', b.dataset.pdf);
  });
}
function deleteInvoice(id){
  const inv=state.invoices.find(x=>x.id===id);
  if(!inv) return toast('No encontrada');
  if(inv.incomeId){
    state.incomesDaily = state.incomesDaily.filter(r=>r.id!==inv.incomeId);
  }
  state.invoices = state.invoices.filter(x=>x.id!==id);
  save();
  toast('Factura eliminada (y su ingreso)');
}
...
function wireQuotesCreate(){
  ...
  $('#addQuoteToday')?.addEventListener('click', ()=>{
    if($('#quoDate')) $('#quoDate').value=todayStr();
  });
  $('#quoteForm')?.addEventListener('submit',(ev)=>{
    ...
    $('#quoSubtotal').textContent='—';
    $('#quoTaxTotal').textContent='—';
    $('#quoGrandTotal').textContent='—';
  });
}
function renderQuotesHistory(q=''){
  const tb=$('#quotesTable tbody');
  if(!tb) return;
  tb.innerHTML='';
  ...
  $$('#quotesTable [data-del]').forEach(b=>{
    b.onclick=()=>{
      state.quotes = state.quotes.filter(x=>x.id!==b.dataset.del);
      save();
      toast('Cotización eliminada');
    };
  });
  $$('#quotesTable [data-edit]').forEach(b=>{
    b.onclick=()=> editQuoteBasic(b.dataset.edit);
  });
  $$('#quotesTable [data-pdf]').forEach(b=>{
    b.onclick=()=> generatePDF('quotes', b.dataset.pdf);
  });
}
function editQuoteBasic(id){
  const i=state.quotes.findIndex(x=>x.id===id);
  if(i<0) return;
...
/* ======== Inicio / Dashboard con balance YTD ======== */
function renderHome(){
  const now=new Date();
  const today=now.toISOString().slice(0,10);
  const yearStart=new Date(now.getFullYear(), 0, 1).toISOString().slice(0,10);
  ...
  if($('#kpiIncomesMonth')){
    $('#kpiIncomesMonth').textContent = fmt(incYTD);
    $('#kpiIncomesMonth').title='Ingresos (YTD)';
  }
  if($('#kpiExpensesMonth')){
    $('#kpiExpensesMonth').textContent = fmt(totalExpYTD);
    $('#kpiExpensesMonth').title='Gastos (YTD)';
  }
  if($('#kpiBalanceMonth')){
    $('#kpiBalanceMonth').textContent = fmt(balanceYTD);
    $('#kpiBalanceMonth').title='Balance (YTD)';
  }
...
async function generatePDF(view="expenses", optionalId=null){
  ...
  function table(headers, rows, startY=42){
    ...
    rows.forEach(r=>{
      r.forEach((c,i)=>doc.text(String(c??'').slice(0,32),14+i*colW,y));
      y+=6;
      if(y>280){
        doc.addPage();
        y=20;
  ...
  function drawInvoiceLike(kind, rec){
    header(kind==='invoice'?'FACTURA':'COTIZACIÓN');
    doc.setFont("helvetica","normal");
    let y=42;
    ...
    const rx=200;
    doc.setFont("helvetica","bold");
    doc.text(kind==='invoice'?'Factura #':'Cotización #', rx-70, ry);
    doc.setFont("helvetica","normal");
    ...
    if(kind==='invoice'){
      doc.setFont("helvetica","bold");
      doc.text("Vence", rx-70, ry);
      doc.setFont("helvetica","normal");
      doc.text(String(rec.dueDate||''), rx-20, ry, {align:'right'});
      ry+=6;
    }else{
      doc.setFont("helvetica","bold");
      doc.text("Válida hasta", rx-70, ry);
      doc.setFont("helvetica","normal");
      doc.text(String(rec.validUntil||''), rx-20, ry, {align:'right'});
      ry+=6;
    }
    ...
    if(rec.terms){
      doc.setFont("helvetica","bold");
      doc.text("Términos:",14,y);
      doc.setFont("helvetica","normal");
      doc.text(String(rec.terms).slice(0,240),14,y+6);
      y+=12;
    }
  }
  ...
  if(view==='invoices' && optionalId){
    ...
    doc.save(`${(business||'Negocio').replace(/\s+/g,'_')}_Factura_${inv.number||''}.pdf`);
    return;
  }
  if(view==='quotes' && optionalId){
    ...
    doc.save(`${(business||'Negocio').replace(/\s+/g,'_')}_Cotizacion_${q.number||''}.pdf`);
    return;
  }
  ...
  if(view==="expenses"){
    ...
    rows=state.reconciliations.map(r=>[
      r.date,
      Number(r.bank||0).toFixed(2),
      Number(r.app||0).toFixed(2),
...
function wireExports(){
  $$('[data-print-view]').forEach(b=>{
    b.addEventListener('click', ()=> generatePDF(b.dataset.printView));
  });
...
function wireSettings(){
  ...
  $('#delLogo')?.addEventListener('click', ()=>{
  ...
  $('#changePIN')?.addEventListener('click', async ()=>{
    const old=$('#pinOld')?.value;
    const n1=$('#pinNew')?.value;
    const n2=$('#pinNew2')?.value;
    ...
    state.settings.pinHash=await sha256(n1);
    save();
...
function renderReconciliations(){
  ...
  $$('#reconTable [data-del]').forEach(b=>{
    b.onclick=()=>{
      state.reconciliations=state.reconciliations.filter(x=>x.id!==b.dataset.del);
      save();
      toast("Eliminado");
    };
  });
  $$('#reconTable [data-edit]').forEach(b=>{
    b.onclick=()=>editReconciliation(b.dataset.edit);
  });
}
function editReconciliation(id){
  const i=state.reconciliations.findIndex(x=>x.id===id);
  if(i<0)return;
  const r=state.reconciliations[i];

  let q=ask(r.date,"Fecha (YYYY-MM-DD)");
  if(q.cancelled)return;
  r.date=q.value;

  q=askNumber(r.bank,"Saldo banco");
  if(q.cancelled)return;
  r.bank=q.value;
  ...
  r.app=calcBalanceApp();
  r.diff=(r.bank||0)-(r.app||0);
  save();
...
function wireReconciliation(){
  $('#reconForm')?.addEventListener('submit',(ev)=>{
    ...
    const rec={
      ...
      bank:Number($('#reconBank').value||0),
      app:calcBalanceApp(),
      diff:0,
      note:$('#reconNote').value
    };
...
function getReconYear(){
  const y=parseInt($('#reconYear')?.value||String(new Date().getFullYear()),10);
...
function renderReconImportTable(){
  const tb=$('#reconImportTable tbody');
  if(!tb) return;
  tb.innerHTML='';
  ...
  if($('#reconImpTotal'))   $('#reconImpTotal').textContent = fmt(total);
...
function wireReconciliationImport(){
  $('#reconImportPreview')?.addEventListener('click', async ()=>{
    const f=$('#reconFile')?.files?.[0];
    if(!f) return toast('Selecciona un CSV');
    ...
    renderReconImportTable();
    toast('Previsualización lista');
  });
...
function uiCloud(){
  if($('#cloudStatus')){
    $('#cloudStatus').textContent = cloud.user
      ? `Conectado como ${cloud.user.displayName||cloud.user.email||cloud.user.uid}`
      : 'No conectado';
  }
  if($('#btnSignIn'))  $('#btnSignIn').style.display  = cloud.user ? 'none' : 'inline-block';
  if($('#btnSignOut')) $('#btnSignOut').style.display = cloud.user ? 'inline-block' : 'none';
  if($('#cloudAuto'))  $('#cloudAuto').checked = !!cloud.autosync;
}
...
async function cloudPull(replace=true){
  ...
  if(!snap.exists()) return toast('No hay datos en la nube');
...
function refreshAll(){
  ...
  renderInvoicesKPI();
  renderQuotesKPI();
  renderInvoicesHistory($('#invSearch')?.value||'');
  renderQuotesHistory($('#quoSearch')?.value||'');
...
function wireAll(){
  ...
  // Logout → regresar al login PIN sin recargar
  $('#logoutBtn')?.addEventListener('click', async ()=>{
    try{ await signOut(auth); }catch(e){}
    // limpiar intentos y forzar pantalla PIN
    localStorage.removeItem(LOCK_KEY);
    toast('Sesión cerrada. Ingresa tu PIN otra vez.');
    updateLoginUI();
  });
  ...
  // 🔐 Siempre se muestra login al iniciar
  updateLoginUI();
}
...
document.addEventListener('visibilitychange', ()=>{
  if(document.visibilityState==='visible' && $('#login')?.classList.contains('visible')){
    forceShowLogin();
  }
});

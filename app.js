...
/* ======== ⬇️ ACTUALIZADO: Inicio con balance corrido (YTD) — no se resetea mes a mes ⬇️ ======== */
function renderHome(){
  const now=new Date();
  const today=now.toISOString().slice(0,10);
  const yearStart=new Date(now.getFullYear(), 0, 1).toISOString().slice(0,10);
  ...
  // KPIs → mostramos YTD (corrido)
  if($('#kpiIncomesMonth')){ $('#kpiIncomesMonth').textContent = fmt(incYTD); $('#kpiIncomesMonth').title='Ingresos (YTD)'; }
  ...
  // Gráfico: permanece como barras por mes (últimos 12) — no alteramos diseño
...
/* ======== ⬆️ FIN actualización renderHome (YTD) ⬆️ ======== */

// nexus-mobile-app.js  (versión limpia con Firebase + sync real)
// ============================================================
// - Usa el MISMO state que Nexus Desktop (STORAGE_KEY = finanzas-state-v10)
// - Se conecta a Firebase con la misma config
// - Lee/escribe incomesDaily, expensesDaily, invoices, etc.
// - "Traer datos de la nube" = jala el mismo documento que Desktop
// - "Enviar datos a la nube" = sube el mismo documento que Desktop
// ============================================================
...
function showScreen(id) {
  ...
  // scroll arriba para evitar que se quede en medio
...
function renderHome() {
  ...
  $("#kpi-balance-today").textContent  = formatMoney(incomeToday - expensesToday);
...
/* Guarda factura en state + incomeDaily, y opcional abre WhatsApp */
function saveInvoice(openWhatsApp) {
  ...
  // También registro en Ingresos (igual que Desktop)
  const income = {
    id: uid(),
    date,
    client: clientName,
...
function initCloudUI() {
  $("#btnSignInMobile")?.addEventListener("click", async () => {
    try {
      ...
      // En iPhone a veces bloquea popup → usamos redirect

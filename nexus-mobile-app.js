...
/* 🔒 UID FIJO DEL DESKTOP (lo que me diste) */
...
function renderKPIHomeMobile(){
  ...
  // === EXACTAMENTE COMO DESKTOP: YTD (corrido del año) ===
...
async function generateInvoicePDFMobile(inv){
  try{
    ...
    doc.setFontSize(16);
    doc.text(business, 14, 18);
    doc.setFontSize(12);
    doc.text(`FACTURA #${inv.number || ""}`, 14, 26);
    doc.setFontSize(10);
    doc.text(`Fecha: ${inv.date || ""}`, 14, 32);
    ...
    doc.text(clientName || "Sin nombre", 14, y+6);
    if (clientPhone) doc.text(`Tel: ${clientPhone}`, 14, y+12);
    y += 20;
    ...
    y += 6;
    const fmtLocal = (n)=> fmt ? fmt(n) : Number(n||0).toFixed(2);

    doc.setFont("helvetica","bold");
    doc.text("Subtotal:", 120, y);
    doc.setFont("helvetica","normal");
    doc.text(fmtLocal(inv.subtotal || 0), 200, y, { align:"right" });
    y += 6;

    doc.setFont("helvetica","bold");
    doc.text("Impuestos:", 120, y);
    doc.setFont("helvetica","normal");
    doc.text(fmtLocal(inv.taxTotal || 0), 200, y, { align:"right" });
    y += 6;

    doc.setFont("helvetica","bold");
    doc.text("TOTAL:", 120, y);
    doc.setFont("helvetica","bold");
    doc.text(fmtLocal(inv.total || 0), 200, y, { align:"right" });
    y += 10;
    ...
    if(inv.note){
      ...
      doc.text("Nota:", 14, y);
      doc.setFont("helvetica","normal");
      doc.text(String(inv.note).slice(0, 200), 14, y+6);
    }

    const blobUrl = doc.output("bloburl");
    window.open(blobUrl, "_blank"); // abre el PDF en una pestaña nueva
  }catch(e){
    console.error("Error generando PDF móvil:", e);
    toast("No se pudo generar el PDF");
  }
}

/* === saveInvoice ACTUALIZADA: guarda, genera PDF y abre WhatsApp === */
async function saveInvoice(openWhatsApp){
  ...
  // 👉 Generar y abrir el PDF de la factura
  try{
...
function initMobileApp(){
  ...
  // Asegurar estado inicial de autosync UI
  const chk = $("#chkAutosyncMobile");
  if(chk) chk.checked = cloud.autosync;
